...
- **A poll or list withdraws the pending notice** for a child or background handle that has **finished**, by design (tickets 06, 10): if the model has read the result, it will not also receive a completion message about it. Polling a **running** child is a status check, not a read, so it leaves that child's completion notice intact — otherwise checking on a running child would silently suppress the notice that it finished.
